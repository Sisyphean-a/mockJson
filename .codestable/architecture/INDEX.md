...
| web/client | 管理 API 的请求、响应和连接错误转换 | `apps/web/src/mock-admin-client.ts` `MockAdminClient` |
| web/state | 页面状态、当前选择、派生查询和服务端加载 | `apps/web/src/use-mock-state.ts` `useMockState` |
...
| web/views | 页面组合根与独立区域视图 | `apps/web/src/App.vue` `apps/web/src/components/` |
...
- 管理配置变更由 `MockConfigService` 统一拥有，路由不得直接修改 `State`；前端由 `MockAdminClient`、`useMockState`、按领域动作 composable 和区域组件组合，不在根组件中实现领域操作。