...
| 范围 | 职责 | 代码锚点 |
| --- | --- | --- |
| server/bootstrap | Fastify 组合根、启动、CORS、静态页面和请求访问边界（含有体代理的流式接管） | `apps/server/src/index.ts` |
...
| server/proxy | 当前包匹配、Mock 返回和未命中流式转发（有请求体时绕过 Fastify 缓冲） | `apps/server/src/proxy.ts` `createProxy` |
| server/logs | 代理请求结果、匹配接口和响应预览的运行态内存记录、环形上限与 ETag 增量游标 | `apps/server/src/request-logs.ts` `apps/server/src/proxy.ts` |
...
| web/client | 浏览器 HTTP 传输接缝、配置/日志管理 API 请求、ETag 日志增量合并、响应和连接错误转换 | `apps/web/src/mock-admin-client.ts` `AdminTransport` `MockAdminClient` |
...
- 管理配置变更由 `MockConfigService` 统一拥有，路由不得直接修改 `State`；配置失败回滚使用序列化快照，避免常驻整份 `structuredClone`。请求日志读取使用 ETag：未变化返回 304，有效游标仍可追溯时只返回新增记录，否则返回重置列表；前端负责合并增量。前端运行地址由 `createRuntimeEndpoints` 统一计算，浏览器调用细节由 `AdminTransport` 吸收，所有管理请求的可用状态由 `useMockState.runAdminRequest` 唯一维护；`App.vue` 只组合区域并唯一展示启动状态，不实现领域操作。