...
- **Match Rule（匹配规则）**：支持任意 Header、URL（fullUrl / host / path）与 HTTP Method，接口内按 AND 或 OR 组合；Header 名称比较不区分大小写。
- **Scenario（响应场景）**：保存完整且合法的 JSON、HTTP 状态码和 0–30000ms 延迟；一个逻辑接口最多有一个当前场景，点击场景立即切换。
- Package、Logical API 和 Scenario 都可在控制台完成创建、修改和删除；新建 Logical API 默认关闭，配置场景与规则后再显式开启，未完成接口不得遮挡其他可用 Mock。
- 接口 `enabled=false` 或没有规则命中时转发当前 Package 的 `targetBaseUrl`；转发保留 JSON 和原始流请求体，保留 `targetBaseUrl` 的基础路径，按目标地址协议选择 HTTP / HTTPS 上游连接，并对无响应上游设置超时。
