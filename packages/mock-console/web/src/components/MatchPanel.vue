...
type MatchPanelController = Pick<ConsoleController, "api" | "updateLogic" | "addingRule" | "editingRuleId" | "openRuleCreate" | "openRuleEdit" | "cancelRuleEdit" | "ruleSource" | "changeSource" | "ruleField" | "urlFields" | "methodFields" | "ruleOperator" | "operators" | "ruleValue" | "ruleHint" | "addRule" | "removeRule" | "matchCollapsed" | "toggleMatchCollapsed">;
  ...
  <section v-if="c.api.value" class="match-card" aria-labelledby="match-title">
    <div class="panel-heading">
      ...
      <div class="match-controls">
        <div v-show="!c.matchCollapsed.value" class="logic-control">
        ...
        <button v-show="!c.matchCollapsed.value" class="add-rule" @click="c.addingRule.value ? c.cancelRuleEdit() : c.openRuleCreate()">{{ c.addingRule.value ? "取消" : "＋ 添加条件" }}</button>
        <button
          class="match-toggle"
          type="button"
          aria-controls="match-body"
          :aria-expanded="!c.matchCollapsed.value"
          :title="c.matchCollapsed.value ? '展开所有逻辑接口的匹配条件' : '折叠所有逻辑接口的匹配条件'"
          @click="c.toggleMatchCollapsed()"
        >
          <span class="match-toggle-chevron" aria-hidden="true"></span>
          {{ c.matchCollapsed.value ? "全部展开" : "全部折叠" }}
        </button>
      </div>
    </div>

    <div v-show="!c.matchCollapsed.value" id="match-body">
